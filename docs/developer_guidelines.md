...
All plugin repositories need to follow the format of `sensu-plugins-app-name`, where `app-name` is the name such as `windows`, `disk-checks`, or `influxdb`.  The exception to the rule are repos used for the site, tooling, documentation, and plugin libraries such as `sensu-plugin` which serves as a base for all ruby plugins and its up and coming python cousin `sensu-plugin-python`.  This is done so that the rake tasks and other automation tools can easily parse Github and effectively work with the 180+ plugin repos.

The majority of the plugins are ruby and while often have other languages it may be desirable to eventually bundle these separately to reduce the dependencies required by other languages. For repositories wanting to adopt this they should include the language name or extension as a suffix such as `sensu-plugin-python` or `sensu-plugins-disk-checks-golang` to allow users to quickly identify what they are about to explore.

### File Naming

All binaries should be prefixed with either `handler-`, `check-`, `metrics-`, or `mutator-`, `filter-`, or `extension-` depending on their primary function. This is done to ensure that a user can quickly tell from the command what the primary function of the script is.  It also makes things easier for infrastructure tools.
...
- All new scripts, modules, or classes must be fully tested. There are well documented examples in the [pagerduty][21] or [dcos][]23 plugin
...
If you would like to make a pull request and the repo does not already exist then please feel free to directly contact a member of the team using our public email address or open an issue in the [Community][19] repo and we will create one for you based on our [Skeleton Repo][22]. As a general rule if you ask to have a repo created you will also be given write access to it by default.
...
[19]: https://github.com/sensu-plugins/community
[20]: https://github.com/sensu-plugins/community/blob/master/HOW_WE_CHANGELOG.md
[21]: https://github.com/sensu-plugins/sensu-plugins-pagerduty
[22]: https://github.com/sensu-plugins/sensu-plugins-skel
[23]: https://github.com/sensu-plugins/sensu-plugins-dcos
